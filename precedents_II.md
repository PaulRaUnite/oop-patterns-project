...
- [x] [find project in archive](#find-project-in-archive)
- [x] [add user](#add-user)
...
3.  Типичный поток событий:
    ...
    | Действие пользователя | Отклик системы |
    |-----------------------|----------------|
    | Пользователь нажимает кнопку _Edit project details_ | `System` отображает форму для редактирования с предзаполненными данными используя геттеры объекта `Project`. |
    | Пользователь изменяет данные проекта, редактирует список участников или их права. Пользователь подтверждает новые данные. | `System` заменяет старые данные новыми с помощью сеттеров `Project`. |
...
3. Типичный поток событий:
    ...
    | Действие пользователя | Отклик системы |
    |-----------------------|----------------|
    | Пользователь вводит параметры поиска | `System` использует метод `find` объекта `Archive` для получения списка проектов подходящий под критерий и выводит этот список Пользователю. |
    | Пользователь выбирает проект из списка | Система отображает информацию по выбранному проекту используя геттеры проекта, выводит контекстные действия по редактированию/окончательному удалению/восстановлению проекта. |
...
4.	Альтернативные потоки событий:
    ...
    | Пользователь выбирает пункт "Restore project" | Система восстанавливает проект, рассылаются уведомления всем его участникам о возобновлении работы. |
